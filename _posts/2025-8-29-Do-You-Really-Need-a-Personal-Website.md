...
If I want to spend an evening just improving something small—maybe the way links look when you hover over them, or adding a new section to showcase a project, then I can do just that. It's rewarding in its own way. Below are some fun examples to improve this blog.

### Code Block

I can create a fancy-looking code block.
...
Here's some C code that calculates two numbers:

```c
// math.c
// Obfuscated C code that does maths
%:include <stdio.h>
%:include <stdlib.h>
%:define V void
%:define I int
...
%:define Z 0XF
%:define M ~(0X1<<0X1F)
%:define N __FUNCTION__
%:define F(x, y) (printf("%-5s(%d, %d): ", N, x, y))
%:define G(p) (sizeof(p)/sizeof(*p))
%:define A(s) atoi(s)
%:define ERR stderr
%:define E(m) (fprintf(ERR, "%s: %s", N, m))
%:define R return
%:define W while
%:define C if

S B = "\x45\x78\x70\x65\x63\x74\x65\x64\x20\x74\x77\x6F\x20\x61\x72\x67\x75\x6D\x65\x6E\x74\x73\x2E\x0A"; I ADD(I x, I y) <% F(x, y); U t; W(y) <% t=x&y; x^=y; y=t<<1; %> R(I)x; %> I SUB(I x, I y) <% F(x, y); I t; W(y) <% t=(~x)&y; x^=y; y=t<<1; %> R(I)x; %> I MULT(I x, I y) <% F(x, y); I t = y^y; W(y--) <% t+=x; %> R(I)t; %> I IDIV(I x, I y) <% F(x, y); C(!y) R(I)M; I t = y^y; W(++t && (x-=y)>=y); R(I)t; %> I Q(I i, I j) <% W(i-- && --j); R!(i || j--); %> V v(I x, I y, I (*p<::>)(), U fs) <% I i = Z^Z; W(i < fs) printf("%i\n", p<:i++:>(x, y)); %> V main(I i, S s<::>) <% I (*p<::>)() = <% ADD, SUB, MULT, IDIV %>; U fs = G(p); Q(i,(Z^0XC)) ? v(A(s<:Z^0XE:>), A(s<:Z^0XD:>), p, fs) : E(B); %>
```

I can make it look like I'm executing the program from a terminal window:
...
```powershell
PS C:\Users\William\Playground> ./math 6 3
ADD  (6, 3): 9
SUB  (6, 3): 3
MULT (6, 3): 18
IDIV (6, 3): 2
PS C:\Users\William\Playground> 
```

Or just display the shell output like this:

```console
root@willnj:~$ ./math 6 3
ADD  (6, 3): 9
SUB  (6, 3): 3
MULT (6, 3): 18
IDIV (6, 3): 2
...
I can also display a directory structure without having to handtype the pipe symbols.
...
I can even have buttons which can say things like "Donate" or "Subscribe" or whatever I want.

{% include cta-button.html 
   text="Call to Action Example 1" 
   href="https://example.com/action" 
   style="primary" %}

{% include cta-button.html 
   text="Call to Action Example 2" 
   href="https://example.com/action" 
   style="secondary" %}

### Tables

And here's a table for different data types in C.
...
If I'm writing a tutorial, I can add these prompts to visually alert the reader.

{% include prompt.html 
   type="tip" 
   content="**Tip:** When using this feature, remember to always save your work frequently. This will prevent any data loss in case of unexpected errors. You can also use keyboard shortcuts like **Ctrl+S** or **Cmd+S** to quickly save. For more detailed instructions, check out [our documentation](https://example.com/docs)." %}

{% include prompt.html 
   type="info" 
   content="**Info:** The system has been updated to include new functionality for managing your tasks. You can now filter by due date, priority, and project, and even save custom views for future use. Remember to refresh the page after making changes to see the latest updates. If you’re unfamiliar with the new layout, hover over the info icons for helpful tooltips." %}

{% include prompt.html 
   type="danger" 
   content="**Warning:** Deleting this item is permanent and cannot be undone. Make sure you have backed up any important data before proceeding. Double-check the item name and details before confirming the deletion. Misclicks can cause serious issues, so proceed with caution. If you’re unsure, consult the [support guide](https://example.com/support) or contact the admin." %}

## Famous People

Here are some famous people in tech.

{% include card_marquee.html source="famous_people_cards" %} 

## Talk About Your Project

You can highlight all of the features of the project you're shipping.

{% include features_list.html source="app_features_example" %}

### Add Badges

Include a list of badges.

{% include badge.html items="windows:brands:Windows,apple:brands:MacOS,linux:brands:Linux" %}
